...
const SHOTS = [
  ...
  { name: '02-me-stickers', hash: '#/stationery', state: { 'jazz-studio-stationery': { kind: 'faces', me: true, pose: 'mix', words: { faces: 'Jazz' } } } },
  { name: '03-diary-cover', hash: '#/stationery', state: { 'jazz-studio-stationery': { kind: 'cover', me: true, pose: 'portrait', words: {} } } },
...
const FOR_README = { '01-hello': 'home.png', '02-me-stickers': 'stickers.png', '04-make-from-a-box': 'box.png', '05-my-brand': 'brand.png', '08-on-a-phone': 'phone.png' };
...
app.whenReady().then(async () => {
  ...
  for (const shot of SHOTS) {
    ...
    await win.webContents.executeJavaScript(
      `localStorage.clear(); ${Object.entries(shot.state).map(([k, v]) => `localStorage.setItem(${JSON.stringify(k)}, ${JSON.stringify(JSON.stringify(v))});`).join('')}`
    );
    ...
    await win.loadURL(`${base}?shot=${shot.name}${shot.hash}`);
