...
| Asset | Licence |
| --- | --- |
| `branding/cutout-*.png`, `branding/sukhi-icon.png`, `build/icon.png`, `build/icons/*.png`, `src/renderer/assets/mascot.png`, the Sukhi Colouring artwork (`apps/colouring/app/*.png`, `apps/colouring/app/assets/sukhi-*.png`), and Jazz's and Sukhi's characters in Jazz's Studio (`apps/studio/app/assets/*.webp`, `apps/studio/app/*.png`, `web/public/assets/playground-studio.png`, `web/public/assets/studio-hello.webp`, `web/public/screenshots/studio/`), likenesses of real children | © Mandeep Singh. **All rights reserved.** Not covered by the code's licence. |
